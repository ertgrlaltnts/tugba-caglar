  ...
  return (
    <>
      <Head>
        ...
        <link
          rel="canonical"
          href={`https://www.tugbacaglar.com/hizmetlerimiz/${data.slug}`}
        ></link>
        ...
        <meta
          property="og:url"
          content={`https://www.tugbacaglar.com/hizmetlerimiz/${data.slug}`}
        />
                ...
                <Col xs={24} sm={24} md={24} lg={18} xl={18}>
                  <Hmp.BlockItem>
                    <Hmp.Image
                      ...
                      alt={`tugba-caglar-hizmet-${data.slug}`}
                      width={600}
                      height={300}
                      priority
                    />
...
export async function getStaticProps({ params }) {
  ...
  const response = await axios.get(
    `${process.env.NEXT_PUBLIC_IP}/api/hizmetlerimizs?filters[slug][$eq]=${params.slug}&populate=deep`
  );
  return {
    props: {
